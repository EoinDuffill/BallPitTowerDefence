...
void BaseEngine::ResizeVector(int increase) {
	int resized = m_ppDisplayableObjects.size();
	m_ppDisplayableObjects.resize((resized + increase));
}

void BaseEngine::pushBack(DisplayableObject* pObject) {
...
void BaseEngine::ResetObjectVector() {
	if (!m_ppDisplayableObjects.empty())
	{
		m_ppDisplayableObjects.clear();
		m_ppDisplayableObjects.push_back(NULL);
...
int BaseEngine::getVectorSize() {
	return m_ppDisplayableObjects.size();
}

void BaseEngine::eraseFromVector(DisplayableObject* pObject) {
	m_ppDisplayableObjects.erase(std::remove(m_ppDisplayableObjects.begin(), m_ppDisplayableObjects.end(), pObject));
}

void BaseEngine::eraseIndexFromVector(int index) {
	m_ppDisplayableObjects.erase(m_ppDisplayableObjects.begin() + (index ));
}
